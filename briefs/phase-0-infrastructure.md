...
**New** (project root: this repository)
...
- `_headers` — Cloudflare Pages headers config, copied verbatim from the maintainer-local `zero-parades` source repo.
- `robots.txt` — allow all crawlers, no sitemap reference yet (Phase 1.5 generates it).
- `scripts/translate.py` — **copied byte-verbatim** from the maintainer-local `zero-parades` source repo. Do not edit `LANGS` in this phase; Phase 1.5 brief sets it.
...
├── scripts/
│   └── translate.py           ← copied verbatim from the maintainer-local zero-parades source repo
└── .claude/                   ← Claude-only
    ├── settings.local.json
    └── skills/
...
Copy the maintainer-local `zero-parades/scripts/translate.py` file byte-verbatim into `scripts/translate.py`. Do **not**:
- Edit the `LANGS` list.
...
TRANSLATE_MODEL_FALLBACK=gpt-5.2
```
...
Copy verbatim from the maintainer-local `zero-parades/_headers` file. If that source file has site-specific paths in it, Codex should flag those for review rather than silently rewriting.
...
2. User reviews against this brief. Specifically check:
   ...
   - `scripts/translate.py` is byte-identical to source (`cmp <maintainer-local-zero-parades>/scripts/translate.py scripts/translate.py` returns nothing).
